...
| :---: | :--- | :--- |
| 🔐 認証<br>(Auth) | Spring Securityを用いた安全なログインと権限(Admin/User)管理 | Spring Security 기반의 안전한 로그인 및 권한(Admin/User) 분리 관리 |
...
| 🤖 AI推薦<br>(AI) | Gemini / OpenAI API連携による、ユーザーの好みに合わせたスポット推薦 | Gemini / OpenAI API 연동으로 사용자 취향에 맞는 핫플레이스 자동 추천 |
...
1. DB Setup: MySQLでデータベースを作成します（例: `kanbee_db`）。 / MySQLから데이터베이스를 생성합니다 (예: `kanbee_db`).
2. Config: `application.properties` にデータベース情報(URL, ID, PW)とAI APIキーを入力します。 / DB 정보(URL, ID, PW)와 AI API 키를 입력합니다.
